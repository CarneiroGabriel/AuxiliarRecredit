...
class ImportadorRecredit:
    ...
    def _parse_pdf_almah(self, text: str) -> list[Boleto]:
        ...
        # Fallback: formato vertical antigo e variações em que cada célula sai em uma linha.
        i = 0
        while i < len(lines):
            if lines[i].upper() in {"ÚNICO", "UNICO"}:
                ...
                # Procura a data dentro do bloco e assume que o token anterior é a unidade.
                date_idx = next((k for k, tok in enumerate(block) if re.match(r"\d{2}/\d{2}/\d{4}$", tok)), None)
                if date_idx is not None and date_idx >= 1:
                    unidade = block[date_idx - 1]
                    venc = block[date_idx]
                    nums = [tok for tok in block[date_idx + 1:] if re.match(r"\(?-?\d[\d\.]*,\d{2}\)?$", tok)]

                    # Em alguns PDFs do Almah o total e o total com desconto são repetidos várias vezes.
                    # Para o layout Nina, as 4 primeiras colunas monetárias após o vencimento são as rubricas:
                    # cota, fundo, gás e chamada de capital. O restante é total repetido.
                    if len(nums) >= 4:
                        boleto = Boleto(unidade=unidade, vencimento=venc)
                        labels = [
                            ("0001", "Cota Condominial"),
                        ...
                        for (cod, hist), valor in zip(labels, nums[:4]):
                            boleto.add_rateio(cod, hist, self._to_money(valor))
                        if boleto.rateios:
                            boletos.append(boleto)
                i = j
